...
fn vec3_from_slice(slice: &[f32; 3]) -> nalgebra::Vector3<f32> {
    nalgebra::Vector3::new(slice[0], slice[1], slice[2])
}
...
impl candle_core::CustomOp1 for Layer {
    ...
    fn bwd(
        ...
        for (i_tri, node2vtx) in tri2vtx.chunks(3).enumerate() {
            ...
            let p0 = vec3_from_slice(del_msh_core::vtx2xyz::to_vec3(vtx2xyz, i0));
            let p1 = vec3_from_slice(del_msh_core::vtx2xyz::to_vec3(vtx2xyz, i1));
            let p2 = vec3_from_slice(del_msh_core::vtx2xyz::to_vec3(vtx2xyz, i2));
            let dw = del_geo_nalgebra::tri3::dw_normal(&p0, &p1, &p2);
            let dw_nrm = vec3_from_slice(del_msh_core::vtx2xyz::to_vec3(dw_tri2nrm, i_tri));
