...
fn vec2_from(slice: &[f32; 2]) -> nalgebra::Vector2<f32> {
    nalgebra::Vector2::new(slice[0], slice[1])
}
    ...
    fn bwd(
        ...
        for i_vtxv in 0..num_vtxv {
            ...
            if info[1] == usize::MAX {
                ...
                let l2 = vec2_from(del_msh_core::vtx2xy::to_vec2(&self.vtxl2xy, i2_loop));
                let i0_site = info[1];
                let i1_site = info[2];
                let s0 = vec2_from(del_msh_core::vtx2xy::to_vec2(site2xy, i0_site));
                let s1 = vec2_from(del_msh_core::vtx2xy::to_vec2(site2xy, i1_site));
                ...
                let dv = vec2_from(del_msh_core::vtx2xy::to_vec2(dw_vtxv2xy, i_vtxv));
                {
                ...
                let s0 = vec2_from(del_msh_core::vtx2xy::to_vec2(site2xy, idx_site[0]));
                let s1 = vec2_from(del_msh_core::vtx2xy::to_vec2(site2xy, idx_site[1]));
                let s2 = vec2_from(del_msh_core::vtx2xy::to_vec2(site2xy, idx_site[2]));
