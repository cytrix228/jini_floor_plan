    ...
    fn cpu_fwd(
        ...
        for i_h in 0..self.img_shape.1 {
            for i_w in 0..self.img_shape.0 {
                ...
                let (ray_org, ray_dir) = del_canvas_core::cam3::ray3_homogeneous(
                    (i_w, i_h),
                    &self.img_shape,
                ...
                .and_then(|t| if t >= 0.0 { Some(t) } else { None })
                .unwrap();
    ...
    fn bwd(
        ...
        for i_h in 0..height {
            for i_w in 0..width {
                ...
                let Some(_t) = del_geo_core::tri3::intersection_against_line(
                    &p0, &p1, &p2, &ray_org, &ray_dir,
                ) else {
                    continue;
                };
                let dw_depth = dw_pix2depth[i_h * self.img_shape.0 + i_w];
                let (_, _, _, dw_p0, dw_p1, dw_p2) =
